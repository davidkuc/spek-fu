---
name: standard-agent
description: 'Standard coding subagent backed by a capable model. Has no built-in workflow — invoke by passing a skill path and inputs in the prompt. The skill provides all logic.'
...
- **web**: Use only when the skill needs external public documentation or standards and the needed context is not already in the workspace; do not use it for open-ended browsing.
...
Invocation: "Just refactor the skill files to be cleaner — use your judgment."
