...
Dispatch `spek-fu/ai/plugins/skf/skills/orch-final-orchestration-validation.md` (full 9-section manifest).
...
Dispatch `spek-fu/ai/plugins/skf/skills/gov-update.md` (full 9-section manifest).
...
Dispatch `spek-fu/ai/plugins/skf/skills/orch-orchestration-summary.md` (compact) using the accumulated inline execution state to write `spek-fu/reports/orchestration-summary-{YYYY-MM-DD-HHmmss}.md` using template at `spek-fu/ai/plugins/skf/templates/orchestration-summary-template.md`.

Then present summary **inline in chat** — user must see outcome without reading a file. (Orchestration summary is available in `spek-fu/reports/orchestration-summary-{YYYY-MM-DD-HHmmss}.md` for reference.)
...
Dispatch `spek-fu/ai/plugins/skf/skills/meta-knowledge-manage.md` (compact, write mode).
...
**Rules**: Distillation only proceeds on explicit signal from Step 5. Pattern creation follows existing schema from `spek-fu/ai/plugins/skf/patterns/pattern-tags.md` and ID auto-increments from `patterns-index.json`.
