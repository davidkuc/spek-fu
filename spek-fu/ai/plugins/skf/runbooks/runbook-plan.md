...
Breaks work into waves, applies complexity gate per task, generates formal plan via `spek-fu/ai/plugins/skf/skills/orch-orchestration-plan.md`, gets user approval, validates before execution. Phase 3 begins after Step 5 succeeds.
...
Dispatch `spek-fu/ai/plugins/skf/skills/orch-skill-resolve.md` (compact manifest).
...
Dispatch `spek-fu/ai/plugins/skf/skills/orch-wave-decompose.md` (compact manifest).
- Input: `intake-context: structured object from Intake Step 9`
...
Dispatch `spek-fu/ai/plugins/skf/skills/orch-orchestration-plan.md`.
...
**Revision cycle**: incorporate feedback → re-dispatch `spek-fu/ai/plugins/skf/skills/orch-orchestration-plan.md` → re-present → re-prompt. Each revision consumes one `maxPlanRevisions` unit. Budget exhausted → escalate with unresolved conflict.
...
Dispatch `spek-fu/ai/plugins/skf/skills/orch-pre-execution-validation.md` (compact) with the approved inline plan. Validate skill IDs, detect wave conflicts, check dependency issues.
