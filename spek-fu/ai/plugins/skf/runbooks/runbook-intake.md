...
Dispatch `spek-fu/ai/plugins/skf/skills/orch-initialize.md` (compact) with `workspace-root` = workspace root path.
...
Dispatch `spek-fu/ai/plugins/skf/skills/meta-knowledge-manage.md` (compact, read mode). Search `knowledge-database.md` for relevant lessons. Runs in parallel with Steps 6 and 7.
...
- **Simple tasks**: Dispatch a single `spek-fu/ai/plugins/skf/skills/orch-index-traversal.md` (compact) from `skf-root-index.json`.
- **Standard or Complex tasks**: Dispatch `spek-fu/ai/plugins/skf/skills/orch-index-traversal.md` (compact) **in parallel**, one per predefined framework branch:
  - Branch 1: `spek-fu/ai/` (skills, scripts, patterns, knowledge) with a compact inline traversal summary for the branch
  - Branch 2: `spek-fu/constitution/` (rules, constraints) with a compact inline traversal summary for the branch
  - Branch 3: `spek-fu/project/` (context, readme, configs) with a compact inline traversal summary for the branch

  > **Spec-context priority**: When `spec-context: true`, the `spec-flow` plugin branch (`spek-fu/ai/plugins/spec-flow/`) should be a priority traversal target within Branch 1 to ensure spec-flow skills and artifacts are surfaced for routing.

Each branch dispatch targets the sub-section of `skf-root-index.json` relevant to that branch and produces its own structured traversal result inline. If a branch summary cannot be kept compact without losing required fidelity, spill is allowed only under `spek-fu/reports/orchestration-spill/`.
...
Dispatch `spek-fu/ai/plugins/skf/skills/orch-pattern-select.md` in parallel with Step 6 (after Step 4 stabilizes).

**Inputs**: `complexity-tier`, `task-signals` (from `pattern-tags.md` canonical vocabulary), `task-description`, `patterns-index-path` (`spek-fu/ai/plugins/skf/patterns/patterns-index.json`).
...
Dispatch `spek-fu/ai/plugins/skf/skills/orch-intake-context.md` (compact). Return a structured intake context object containing:
