...
Wave summaries are returned inline by the `orch-wave-verification` skill after each wave completes. The orchestrator carries this state forward to confirm wave success before dispatching the next wave. If a spill fallback is required, it must live under `spek-fu/reports/orchestration-spill/`.
