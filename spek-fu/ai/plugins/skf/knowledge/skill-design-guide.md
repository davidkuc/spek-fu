...
- **Environment Preflight** subsection (required): If `env` is `devcontainer`, read `spek-fu/ai/plugins/skf/knowledge/devcontainer-guidelines.md` fully and apply DCR-01–09 rules before Step 1. If `env` is `host`, no additional action required.
...
**[GSR-13]** A skill file should be a workflow, not a reference manual. Put deep domain knowledge in co-deployed files in `spek-fu/ai/plugins/skf/knowledge/` and reference them from the skill.
...
**Fix**: For stubs — expand the workflow with concrete steps, an output template, and a Rules section. For oversized skills — identify content that belongs in a co-deployed reference file under `spek-fu/ai/plugins/skf/knowledge/` and move it there.
...
**Fix**: Extract the shared content into a named knowledge file in `spek-fu/ai/plugins/skf/knowledge/` and instruct sub-agents to load it via `read_file` at the start of their prompt. Remove the inline copy from each sub-agent prompt.
...
**Check**: Every tag in the skill's frontmatter `tags:` list must exist in the canonical vocabulary in `spek-fu/ai/plugins/skf/knowledge/skill-tags.md`. Flag ❌ if any tag is not found in that vocabulary.
