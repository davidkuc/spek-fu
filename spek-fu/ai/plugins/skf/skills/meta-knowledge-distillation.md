...
anti-scope: "Does NOT record or retrieve lessons — use meta-knowledge-manage for that. Does NOT implement tasks. Modifies `knowledge-database.md` (merging duplicates and removing promoted lessons) and `spek-fu/ai/plugins/skf/patterns/` (writing new pattern files). Does not modify any other files."
tags:
  - "meta"
  - "patterns"
  - "lifecycle"
  - "framework"
inputs:
  - "Knowledge database file path (optional, defaults to spek-fu/ai/plugins/skf/knowledge/knowledge-database.md)"
  - "env: runtime environment passed by the orchestrator — 'devcontainer' or 'host'"
outputs:
  - "Execution status: ok, blocked, or fail"
  - "Duplicate/similar lessons merged in knowledge-database.md"
  - "Pattern file(s) written at spek-fu/ai/plugins/skf/patterns/PTxxx-name.md"
...
**Scope boundary**: This skill modifies `knowledge-database.md` (merging duplicates and removing promoted lessons) and `spek-fu/ai/plugins/skf/patterns/` (writing new pattern files). It does not record new lessons or perform any implementation work. For recording or surfacing lessons, use **meta-knowledge-manage**.
...
3. ALWAYS use only tags, categories, and signals from `spek-fu/ai/plugins/skf/patterns/pattern-tags.md` when writing pattern files — WHY: non-canonical tags break index routing and discovery.
...
- The knowledge database path defaults to `spek-fu/ai/plugins/skf/knowledge/knowledge-database.md` if not provided.
...
For each approved cluster, write `spek-fu/ai/plugins/skf/patterns/PTxxx-name.md` using this template:
...
```json
{
  "name": "PTxxx",
  "type": "file",
  "path": "spek-fu/ai/plugins/skf/patterns/PTxxx-name.md",
