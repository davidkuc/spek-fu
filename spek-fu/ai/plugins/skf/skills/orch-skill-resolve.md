...
```
python3 spek-fu/ai/scripts/python/gather-skills.py --format json
```
...
- Parse the JSON output. The result has the shape:
  ...
  ```
- Use `result.skills` as the unified skill catalog — it is already merged across all plugins and filtered against the ignore list from `spek-fu/ai/plugins/skf/skills/config.json`.
...
<tools>
- **run_in_terminal**: Step 2 only — run `python3 spek-fu/ai/scripts/python/gather-skills.py --format json` to obtain the pre-filtered skill catalog.
...
2. Run `python3 spek-fu/ai/scripts/python/gather-skills.py --format json`; parse JSON output (e.g. total=30, ignored=23, available=7)
...
| ID | Path | Description | Recommended Tier | Dispatch Variant | Anti-scope |
|----|------|-------------|------------------|-----------\u200b---|------------|
| gov-update | spek-fu/ai/plugins/skf/skills/gov-update.md | Applies targeted changes to a governance tier with per-change user approval | fast-agent | full | It does NOT perform analysis, modify source code, or touch iteration in-progress files |
| impl-implement | spek-fu/ai/plugins/skf/skills/impl-implement.md | ... | fast-agent | full | ... |
...
- **ID**: gov-update
- **Path**: spek-fu/ai/plugins/skf/skills/gov-update.md
...
2. Step 2: runs `python3 spek-fu/ai/scripts/python/gather-skills.py --format json`; script exits with code 1.
