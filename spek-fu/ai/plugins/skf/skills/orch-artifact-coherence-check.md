...
<examples>
<example>
Input: completed_wave=2, next_wave=3, inline plan. Wave 2 declares output `spek-fu/reports/orchestration-summary-{timestamp}.md`. Wave 3 declares input `spek-fu/reports/orchestration-summary-{timestamp}.md`. The file exists on disk. Path contract is satisfied.
Expected output: Status OK, checks table shows the file as present, incoherences empty. Report returned inline.
</example>

<example>
Input: completed_wave=1, next_wave=2, inline plan. Wave 1 declares output `spek-fu/reports/orchestration-summary-{timestamp}.md`. Wave 2 declares input `spek-fu/reports/orchestration-summary-{timestamp}.md`. The file does not exist on disk.
Expected output: Status BLOCKED, checks table shows `spek-fu/reports/orchestration-summary-{timestamp}.md` as exists=no, incoherences list `missing-output`. Report returned inline.
</example>
