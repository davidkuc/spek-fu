...
version: 1.0
description: "Composes the wave summary state inline, then independently verifies a completed execution wave by comparing the wave plan (expected tasks and outputs) against the wave summary (actual results). USE FOR: documenting what a wave executed, confirming produced expected artifacts, and catching structural and semantic gaps before moving to the next wave. Produces both wave summary and verification state inline."
...
inputs:
  ...
  - "Inline wave summary content (may be omitted if task records are provided for Step 0) (optional)"
  - "env: runtime environment passed by the orchestrator — 'devcontainer' or 'host'"
outputs:
  - "Execution status: ok, blocked, or fail (WAVE PASS or WAVE FAIL)"
  - "Structured wave summary state"
...
Receives the wave plan (expected tasks and output file specifications), wave execution task records, and (optionally) an existing wave summary for a single execution wave. In Step 0, composes the wave summary state from the task records. Then performs two verification passes — structural and semantic — and returns a Verification Report with a per-task verdict and an overall wave status.
...
- Return the Verification Report and wave summary state inline so the orchestrator can carry them forward directly.
...
Before running verification checks, compose the wave summary state.

1. Using the wave execution task records provided as input, compose the wave summary following the template at `spek-fu/ai/plugins/skf/templates/wave-summary-template.md`.
...
4. Log: `Step 0 complete — wave summary state assembled`
...
- If the summary state is missing, stop before structural checks and emit the fail-fast report.

## Done conditions

- **blocked run** is done when the missing-summary fail-fast report is returned.
- **verification run** is done when every task from the wave plan has a structural verdict, every structurally passing task has a semantic verdict, and the inline report includes both wave summary and verification state.
...
Read the wave plan (inline or from file path if provided as a path). Read the inline wave summary content. Parse the markdown content to extract the task list from the Tasks table and the result list from the structured sections of the wave summary.

If the wave summary is absent, immediately return a FAIL report: "Wave summary state not found — wave may not have completed execution."
...
The skill is complete when the Verification Report block and wave summary state have been returned.
...
Task W2-T1: Update skills-index.json
  ...
  Expected outputs: [spek-fu/ai/plugins/skf/skills/skills-index.json]
```
...
**Verification**:
- Structural: `spek-fu/ai/plugins/skf/skills/skills-index.json` → file exists and is valid JSON → PASS
...
Task W3-T2: Write orch-wave-verification.md skill spec
  ...
  Expected outputs: [spek-fu/ai/plugins/skf/skills/orch-wave-verification.md]
```
...
### Failure details
**W3-T2**: File `spek-fu/ai/plugins/skf/skills/orch-wave-verification.md` contains unfilled placeholder `[PLACEHOLDER]` at constraints section. Content appears to be an incomplete draft.
