...
version: 1.0
description: "Compiles and writes the orchestration summary document from inline wave summaries and the orchestration plan. USE FOR: generating the final timestamped orchestration summary report under spek-fu/reports/ at the close of a workflow run; filling the summary template from accumulated orchestration state; returning the completed summary inline for chat display."
...
Loads the orchestration summary template, fills all `{placeholder}` fields from inline wave summaries and the orchestration plan, writes the completed document to `spek-fu/reports/orchestration-summary-{YYYY-MM-DD-HHmmss}.md`, and returns the full summary text for inline chat display.

**Scope boundary**: This skill reads the template and caller-provided orchestration state; writes exactly one timestamped summary report under `spek-fu/reports/`. It does NOT modify any wave summary state, the orchestration plan, or any other artifact. Placeholder values that cannot be resolved are replaced with `N/A` or `(not recorded)` — the output file is always written.
...
- **State detection**: Scan `spek-fu/reports/` for any existing `spek-fu/reports/orchestration-summary-*.md` using `file_search`. If prior summaries exist, note that a previous run has already been recorded — proceed to generate a fresh timestamped summary (each invocation produces a uniquely named file; no overwrite risk). If no prior summaries exist → proceed with normal execution.

---

## Step 1 — Load template

Attempt to read `spek-fu/ai/plugins/skf/templates/orchestration-summary-template.md` using `read_file`.
...
Write the fully resolved template string to `spek-fu/reports/orchestration-summary-{YYYY-MM-DD-HHmmss}.md` using `create_file` as appropriate. Create the `spek-fu/reports/` directory if it does not exist.
...
```
## Orchestration Summary — spek-fu/reports/orchestration-summary-{YYYY-MM-DD-HHmmss}.md
```
...
| output_path | `spek-fu/reports/orchestration-summary-{YYYY-MM-DD-HHmmss}.md` |
...
- The `spek-fu/reports/` directory must be created if it does not exist before writing the output file.
</output_format>
...
Expected output: Template read; wave-1/2/3-summary.md files read; all placeholders filled; spek-fu/reports/orchestration-summary-{timestamp}.md written; full text returned inline. Status: ok.
</example>
