...
**Scope boundary**: This skill traverses indexes and reports paths only. It does NOT read resolved resource files, modify source or index files, or ask the user questions. It returns the Navigation Result report inline for downstream orchestration use and may spill under `spek-fu/reports/orchestration-spill/` only when compaction is insufficient.
...
└─ spek-fu/ai/ai-index.json (Level 1)
  └─ spek-fu/ai/plugins/plugins-index.json (Level 2)
    └─ spek-fu/ai/plugins/{name}/{name}-index.json (Level 3, one per plugin)
...
```
skf-root-index.json (Level 0)
├─ spek-fu/constitution/constitution.md (Terminal)
├─ spek-fu/project/project.md (Terminal)
└─ spek-fu/reports/ (direct folder scan for report files)
```
...
- Signals pointing to framework resources (skills, agents, flows, commands, templates): follow the `spek-fu/ai/` child.
- Signals pointing to governance, constraints, or principles: follow the `spek-fu/constitution/` child.
- Signals pointing to project requirements or technical specs: follow the `spek-fu/project/` child.
- Signals pointing to generated analysis outputs or prior reports: follow the `spek-fu/reports/` child.

If the query signals more than one branch, traverse all relevant branches and merge results.

**Script**: Use `resolve-skf-index-chain.py` to follow the index chain from root: `python3 spek-fu/ai/scripts/python/resolve-skf-index-chain.py --root-index skf-root-index.json --json`
...
- **`spek-fu/ai/` branch**: Read `spek-fu/ai/ai-index.json` → find the `plugins/` child → read `spek-fu/ai/plugins/plugins-index.json` → iterate over all plugin children; for each plugin read its root index (`spek-fu/ai/plugins/{name}/{name}-index.json`). Within each plugin index, identify only sub-folders relevant to the query and read only those.
- **`spek-fu/constitution/`**: Read `spek-fu/constitution/constitution.md` directly.
- **`spek-fu/project/`**: Read `spek-fu/project/project.md` directly.
- **`spek-fu/reports/`**: Inspect the `spek-fu/reports/` folder directly and evaluate report filenames for relevance.
...
- Extract children from a single index: `python3 spek-fu/ai/scripts/python/get-skf-index-children.py --index-path spek-fu/ai/ai-index.json --json`
- Verify target paths exist: `python3 spek-fu/ai/scripts/python/validate-skf-index-targets.py --input-file children.json --json`
- Pre-filter files by frontmatter fields (first pass): `python3 spek-fu/ai/scripts/python/search-by-frontmatter.py --root spek-fu/ai/plugins/skf/ --group <group> --format json`
...
**Script**: Use `search-by-frontmatter.py` as a first-pass filter when the query maps cleanly to frontmatter fields (group, tier, tags): `python3 spek-fu/ai/scripts/python/search-by-frontmatter.py --root spek-fu/ai/plugins/skf/ --group <group> --format json`. For queries requiring semantic reasoning or cross-branch navigation, rely on AI relevance scoring instead.
...
Return the Navigation Result report inline. If the result cannot fit within the compact output budget after compaction, spill to the supplied output path or to `spek-fu/reports/orchestration-spill/traversal-report.md` and return that path.
...
- **create_file**: Only for optional spill fallback under `spek-fu/reports/orchestration-spill/` or an explicitly supplied spill path. No other files may be written.
</tools>
...
Expected output: Traverses the `spek-fu/constitution/` branch, reads the constitution document, and returns the governance-related constitution content. No framework skill or project files are included.
</example>
