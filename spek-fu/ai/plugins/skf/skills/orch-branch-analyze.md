...
- **create_file**: Use only for optional spill fallback under `spek-fu/reports/orchestration-spill/` when the branch analysis cannot fit inline.
...
<examples>
<example>
Input: branch-file="spek-fu/reports/orchestration-spill/framework-traversal-branched-{###}.json" with domain="skills", context="Add a new utility skill for removing stale workflow temp files", 6 entries including two utility skills, one implementation skill, skills-index.json, and README.md.
...
<example>
Input: branch-file="spek-fu/reports/orchestration-spill/framework-traversal-branched-{###}.json" with domain="constitution", context="Add a new utility skill", 3 entries.
