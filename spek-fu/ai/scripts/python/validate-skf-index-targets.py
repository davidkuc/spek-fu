...
Usage:
    cat children.json | python3 spek-fu/ai/scripts/python/validate-skf-index-targets.py
    python3 spek-fu/ai/scripts/python/validate-skf-index-targets.py --input-file children.json
    python3 spek-fu/ai/scripts/python/validate-skf-index-targets.py --input-file children.json --json
    python3 spek-fu/ai/scripts/python/validate-skf-index-targets.py --all-plugins
    python3 spek-fu/ai/scripts/python/validate-skf-index-targets.py --all-plugins --json
...
Input: JSON array of { name, path, type?, index? } objects (from stdin or file),
    ...
    spek-fu/ai/plugins/plugins-index.json (covers skf, spec-flow, and any future plugins).
...
def collect_all_plugin_items(repo_root: str) -> list[dict]:
    ...
    Reads ``spek-fu/ai/plugins/plugins-index.json`` and recursively traverses each
    ...
    """
    items: list[dict] = []
    plugins_index_path = Path(repo_root) / "spek-fu/ai/plugins/plugins-index.json"
...
def main() -> int:
    ...
    parser.add_argument(
        ...
        help=(
            ...
            "spek-fu/ai/plugins/plugins-index.json (ignores stdin / --input-file)"
        ),
    )
