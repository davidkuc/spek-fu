...
_ROOT_VIRTUAL_CHILDREN: dict[str, dict[str, str]] = {
    "ai": {
        "type": "folder",
        "path": "spek-fu/ai/",
        "index": "spek-fu/ai/ai-index.json",
    },
    "constitution": {
        "type": "file",
        "path": "spek-fu/constitution/constitution.md",
    },
    "project": {
        "type": "file",
        "path": "spek-fu/project/project.md",
    },
    "reports": {
        "type": "folder",
        "path": "spek-fu/reports/",
    },
    "features": {
        "type": "folder",
        "path": "spek-fu/features/",
    },
}
...
def _discover_plugin_catalogs(repo_root: Path) -> list[dict[str, Any]]:
    ...
    for plugin in plugins_index.get("children", []):
        ...
        for child in plugin_index.get("children", []):
            ...
            child_index_name = Path(child.get("index", "")).name
            if not child_name or not child_index_name:
                continue

            child_folder  = f"{plugin_folder}/{child_name}"
            child_index_rel = f"{child_folder}/{child_index_name}"
...
def child_rel_path(folder: str, name: str, is_dir: bool) -> str:
    ...
    Files:   "spek-fu/constitution/constitution.md"
    Folders: "spek-fu/constitution/"  (trailing slash convention)
    Root children: "README.md" / "spek-fu/constitution/"
    """
...
def scan_root_contract(excluded: list[str], index_filename: str) -> dict[str, bool]:
    """Return root index items, preserving logical branches under spek-fu/."""
    items = scan_folder("", excluded + ["spek-fu"], index_filename)
    wrapper_root = REPO_ROOT / "spek-fu"
    if wrapper_root.is_dir():
        for name, entry in _ROOT_VIRTUAL_CHILDREN.items():
            target = REPO_ROOT / entry["path"]
            if target.exists():
                items[name] = entry["type"] == "folder"
    return items


def scan_catalog_items(spec: dict[str, Any], index_filename: str) -> dict[str, bool]:
    """Return direct child items for the given catalog spec."""
    if spec["builder"] == "root":
        return scan_root_contract(spec["excluded"], index_filename)
    return scan_folder(spec["folder"], spec["excluded"], index_filename)
...
def build_root_entry(
    ...
    """Build root index entries, preserving logical branches under spek-fu/."""
    if name in _ROOT_VIRTUAL_CHILDREN:
        virtual = _ROOT_VIRTUAL_CHILDREN[name]
        entry: dict[str, Any] = {
            "name": existing["name"] if existing else name,
            "type": virtual["type"],
            "description": (existing or {}).get("description", ""),
...
def validate_index(spec: dict[str, Any]) -> list[dict[str, Any]]:
    ...
    disk_items = scan_catalog_items(spec, index_path.name)

    disk_path_to_name: dict[str, str] = {}
    disk_path_to_isdir: dict[str, bool] = {}
    for name, is_dir in disk_items.items():
        logical_path = builder(folder, name, is_dir, None)["path"]
        disk_path_to_name[logical_path] = name
        disk_path_to_isdir[logical_path] = is_dir
...
def sync_index(
    ...
    # ── Scan disk ────────────────────────────────────────────────────────────
    disk_items: dict[str, bool] = scan_catalog_items(spec, index_filename)
