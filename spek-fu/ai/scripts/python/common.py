...
def find_skf_root(start_dir: str | Path | None = None) -> str | None:
    """Find the SKF project root by looking for the root index marker.
    
    Searches upward from start_dir (or current directory) until it finds
    the repository root marker file ``skf-root-index.json``.
    ...
    while True:
        root_marker = current / 'skf-root-index.json'
...
def get_repo_root() -> str:
    ...
    # Navigate from spek-fu/ai/scripts/python to repository root
