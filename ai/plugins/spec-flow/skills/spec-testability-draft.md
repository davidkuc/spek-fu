...
- Resolve feature-dir: if not supplied, apply Branch Detection from `branch-detection.md`. If unable to read that file, run `git branch --show-current`, extract numeric prefix and feature name, construct as `features/<branch-name>`. If command fails, stop and report `blocked`.
...
Read ai/plugins/spec-flow/skills/config.json and use config["spec-testability-draft"].maxFindings to cap Section 5 findings; default to 40 if key absent.
