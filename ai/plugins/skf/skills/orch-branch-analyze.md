...
tags:
  ...
  - "analysis"
inputs:
  - "Path to one framework-traversal-branched-###.json file in .orchestration-temp/ (required)"
  - "Free-form context summary of the orchestration request (optional)"
  - "env: runtime environment passed by the orchestrator — 'devcontainer' or 'host'"
outputs:
  - "framework-branch-analysis-###.json written to .orchestration-temp/"
...
Receives one `framework-traversal-branched-###.json` branch file and a context summary, reads every file listed in the branch, assesses each file's relevance against the context, and writes a `framework-branch-analysis-###.json` report to `.orchestration-temp/`. Returns an Analysis Summary block to the caller.

**Scope boundary**: This skill reads and analyzes files in a single branch only. It does NOT group or split entries, does NOT traverse indexes, and does NOT modify any framework file. Each instance is designed to run concurrently with other branch-analysis instances — one instance per branch file.

<!-- SECTION 2: Non-negotiable constraints -->
<constraints>
IMPORTANT: These rules override all other instructions and apply throughout every step.
1. Read framework files only — do not modify, create, or delete framework files; writing is permitted ONLY to `.orchestration-temp/` output files. WHY: QA analysis and mutation are separate responsibilities; mixing them corrupts the audit trail.
...
5. ALWAYS write the analysis file to `.orchestration-temp/` using the same sequence number as the input branch file. WHY: the orchestrator matches analysis files to branch files by sequence number; mismatches break result aggregation.
...
- Resolve the branch file path and the derived output path before reading entries.
- Detect whether the run is first-write, refresh, or already-complete by checking whether `.orchestration-temp/framework-branch-analysis-{###}.json` already exists for the same branch number.
- If an existing analysis file is present for the same branch number, refresh that file in place rather than creating a duplicate.
...
Write the object to `.orchestration-temp/framework-branch-analysis-{###}.json`, where `{###}` matches the `branch` field from the branch file. Use `create_file` on first write; use `replace_string_in_file` to refresh an existing file in place.
...
Return the Analysis Summary block defined in `<output_format>`. The skill is complete when `.orchestration-temp/framework-branch-analysis-{###}.json` has been written or refreshed and the Analysis Summary block has been returned.

Detect prior output state manually by checking whether `.orchestration-temp/framework-branch-analysis-{###}.json` already exists and is non-empty before writing. Aggregate the overall verdict manually from the completed finding set: if every analyzed entry is acceptable, return `ok`; if the branch cannot be analyzed because required files are unreadable, return `blocked`; otherwise return `fail` with the concrete blocking findings.
...
- **create_file**: Write the analysis output file to `.orchestration-temp/` on first write.
- **replace_string_in_file**: Refresh an existing analysis file in place when the same branch number already has an output file in `.orchestration-temp/`.
...
| wave | `N` |
| step | `N.M` |
| output_path | `.orchestration-temp/framework-branch-analysis-###.json` |
| summary | one-line summary of what was done |

Return the Analysis Summary block to the caller. Do not add explanatory prose beyond what the template specifies. The full structured findings are in the written JSON file; the inline block provides the orchestrator-facing summary only.
...
Analysis written to: .orchestration-temp/framework-branch-analysis-{###}.json
```
...
<examples>
<example>
Input: branch-file=".orchestration-temp/framework-traversal-branched-{###}.json" with domain="skills", context="Add a new utility skill for removing stale workflow temp files", 6 entries including two utility skills, one implementation skill, skills-index.json, and README.md.
Expected output: Reads branch file. Reads all 6 files fully. Scores the two utility skills as `high` (both are directly relevant to authoring a new utility skill). Scores skills-index.json as `high` (registration pattern needed). Scores the implementation skill as `moderate` (useful background for skill structure). Scores README as `low`. Writes `.orchestration-temp/framework-branch-analysis-{###}.json` with all 6 findings, branch_conclusion referencing the most relevant files. Returns Analysis Summary table with 6 rows.
</example>

<example>
Input: branch-file=".orchestration-temp/framework-traversal-branched-{###}.json" with domain="constitution", context="Add a new utility skill", 3 entries.
