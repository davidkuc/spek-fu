...
Reads the caller-provided pattern-select result, resolves each required and advisory pattern ID to its full file body via `ai/plugins/skf/patterns/patterns-index.json`, and returns the assembled bundle inline for use by subagents dispatched in that wave.
...
4. Return the bundle inline as the canonical output. Only spill to `reports/orchestration-spill/` when the compact output budget cannot preserve the required full pattern bodies. WHY: the orchestration contract is inline-first.
...
- When `output_path` is present, it must point to a spill artifact under `reports/orchestration-spill/`.
</output_format>
