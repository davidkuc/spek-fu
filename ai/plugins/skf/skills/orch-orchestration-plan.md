...
6. For each skill, derive its path using the convention `ai/plugins/skf/skills/{skill-id}.md`. **Always derive the agent tier from the `tier` field in `ai/plugins/skf/skills/skills-index.json`. Do not apply prefix-based fallback tier rules.**
...
|------|--------|----------|
| Step 1 item 6: Derive skill paths and agent tiers | Manual resolution | Derive each skill path with the documented convention `ai/plugins/skf/skills/{skill-id}.md`, then read the provided skill registry or skills index excerpt to confirm the agent tier. |
...
**Skill path**: `ai/plugins/skf/skills/{skill-id}.md`
