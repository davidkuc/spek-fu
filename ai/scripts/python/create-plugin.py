#!/usr/bin/env python3
"""Spek-Fu — Scaffold a new plugin folder under ai/plugins/.
...
#   ai/scripts/python/create-plugin.py → ai/scripts/python → ai/scripts → ai → root
REPO_ROOT = Path(__file__).resolve().parents[3]

PLUGINS_INDEX = "ai/plugins/plugins-index.json"
...
def _build_plugin_index(name: str) -> dict:
    return {
        ...
        "description": f"Index of ai/plugins/{name} — {name} plugin",
        "path": f"ai/plugins/{name}/",
        "children": [
            {
                "name": "knowledge",
                "type": "folder",
                "description": f"Knowledge resources for the {name} plugin",
                "path": f"ai/plugins/{name}/knowledge/",
                "index": f"ai/plugins/{name}/knowledge/knowledge-index.json",
            },
            {
                "name": "skills",
                "type": "folder",
                "description": f"Skill definitions for the {name} plugin",
                "path": f"ai/plugins/{name}/skills/",
                "index": f"ai/plugins/{name}/skills/skills-index.json",
            },
            {
                "name": "templates",
                "type": "folder",
                "description": f"Scaffolding templates for the {name} plugin",
                "path": f"ai/plugins/{name}/templates/",
                "index": f"ai/plugins/{name}/templates/templates-index.json",
            },
        ],
    }
...
def _build_knowledge_index(name: str) -> dict:
    return {
        ...
        "description": f"Index of ai/plugins/{name}/knowledge",
        "path": f"ai/plugins/{name}/knowledge/",
        "children": [],
    }
...
def _build_skills_index(name: str) -> dict:
    return {
        ...
        "description": f"Index of ai/plugins/{name}/skills — skill metadata for routing and discovery",
        "path": f"ai/plugins/{name}/skills/",
        "children": [],
    }
...
def _build_templates_index(name: str) -> dict:
    return {
        ...
        "description": f"Index of ai/plugins/{name}/templates — scaffolding templates",
        "path": f"ai/plugins/{name}/templates/",
        "children": [],
    }
...
def main() -> int:
    parser = argparse.ArgumentParser(
        description="Scaffold a new plugin folder under ai/plugins/",
    ...
    plugin_dir = REPO_ROOT / "ai/plugins" / name
    ...
    dirs_to_create: list[str] = [
        f"ai/plugins/{name}/",
        f"ai/plugins/{name}/knowledge/",
        f"ai/plugins/{name}/skills/",
        f"ai/plugins/{name}/templates/",
    ]
    files_to_create: list[tuple[str, dict]] = [
        (f"ai/plugins/{name}/{name}-index.json",              _build_plugin_index(name)),
        (f"ai/plugins/{name}/knowledge/knowledge-index.json", _build_knowledge_index(name)),
        (f"ai/plugins/{name}/skills/skills-index.json",       _build_skills_index(name)),
        (f"ai/plugins/{name}/templates/templates-index.json", _build_templates_index(name)),
    ]
    ...
    new_entry = {
        ...
        "description": "",
        "path": f"ai/plugins/{name}/",
        "index": f"ai/plugins/{name}/{name}-index.json",
    }
